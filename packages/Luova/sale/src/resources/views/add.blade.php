...
@section('title',  (!empty($fdata->id) ? 'Edit Sale' : 'New Sale'))
@section('content')
...
<section class="content">
    <div class="container-fluid">
        ...
        {{ Form::open(['method' => 'POST', 'route'=>'sale.store']) }}
        <div class="row">
            <div class="col-md-12">
                <div class="card card-default">
                    <div class="card-header">
                        <h3 class="card-title mb-0">{{  (!empty($fdata->id) ? 'Edit Sale' : 'New Sale') }} </h3>
                    </div>
                    <!-- @dump($fdata) -->
                    <!-- /.card-header -->
                    <!-- form start -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="row" id="RecieptHead">
                            <div class="col-md-8 ">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-4">
                                        {{ Form::select('party_ac', AccountSelect([28,19,7]),(!empty($fdata->party_ac) ? $fdata->party_ac : NULL), ['class' => (($errors->has('party_ac')? 'is-invalid': ''). ' form-control select2'), 'placeholder' => 'Select' ]) }}
                                        @error('party_ac')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="form-group row">
                                    ...
                                    {{ Form::label('sale_ledger', 'Sale ledger',['class' => 'col-sm-2 col-form-label'])}}
                                    <div class="col-sm-4">
                                        {{ Form::select('sale_ledger', AccountSelect(2),(!empty($fdata->sale_ledger) ? $fdata->sale_ledger : NULL), ['class' => (($errors->has('sale_ledger')? 'is-invalid': ''). ' form-control select2'), 'placeholder' => 'Select' ]) }}
                                        @error('sale_ledger')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="col-4">
                                <div class="form-group row">
                                    {{ Form::label('id', 'Sale invoice no',['class' => 'col-sm-6 text-right col-form-label'])}}
                                    <div class="col-sm-6">
                                ...
                                <div class="form-group row">
                                    {{ Form::label('date', 'Price type',['class' => 'col-sm-6 text-right col-form-label'])}}
                                <div class="form-check form-check-inline">
                                    <label class="form-check-label" >
                                      
                                        {{ Form::radio('price_type','MRP',((!empty($fdata->price_type) && $fdata->price_type == 'MRP') ? true : false), ['class'=>'form-check-input']) }} MRP
                                    </label>
                                  </div>
                                  <div class="form-check form-check-inline">
                                    <label class="form-check-label">
                               
                                        {{ Form::radio('price_type','DP',((!empty($fdata->price_type) && $fdata->price_type == 'DP') ? true : true), ['class'=>'form-check-input']) }} DP
                                    </label>
                                  </div>
                                  <div class="form-check form-check-inline">
                                    <label class="form-check-label">
                                        {{ Form::radio('price_type','TP',((!empty($fdata->price_type) && $fdata->price_type == 'TP') ? true : false), ['class'=>'form-check-input']) }} TP
                        ...
                        <table class="table  table-sm table-striped">
                            ...
                            <tbody id="appendRow">
                                ...
                                @include('sale::part.old_form', ['rows' => $olds])
                                @php
                                ...
                                @include('sale::part.edit_form', ['rows' => $fdata->details])
                                @php
                                ...
                                $rn = 5;
                                @endphp
                                @include('sale::part.default_form', ['rows' => $default_form])
...
<script>
    ...
    function append_row() {
        var $ = jQuery;
            ...
            let route = "{{ route('sale.ajax.rowitem') }}";
            var rows = count;
            $.ajax({
                type:'GET',
                url:route,
    ...
    function selectProduct(self) {
        var $ = jQuery;
        var pid = $(self).val();
        var dumplicat = duplicateItemCheck();
        ...
        if(dumplicat != true){
            ...
            if(price_type){
                //alert(pid);
                if(pid){
                    let route = "{{ route('sale.ajax.product') }}";
                    var rows = count;
                    $.ajax({
                        type:'GET',
                        url:route,
                        data:{
                            pid:pid,price_type:price_type
                        },
                        success:function(data){
                            if(data.valid){
                                //purchase_price
                                //alert(data.purchase_price)
                                $(self).closest('tr').find('.item-rate').val(data.sale_price);
                                $(self).closest('tr').find('.item-stock').val(data.stock);
                                $(self).closest('tr').find('.item-unit').html(data.unit);
                                $(self).closest('tr').find('.type-rate').html(data.price_type);
    ...
    function rowUnSelect(self) {
        $(self).val(null).trigger('change');
        $(self).closest('tr').find('.item-rate').val(null); 
        $(self).closest('tr').find('.item-rate').prop("readonly",true);
        $(self).closest('tr').find('.item-qty').val(null); 
        $(self).closest('tr').find('.item-qty').attr("readonly",true); 
        $(self).closest('tr').find('.item-stock').val(null);
        $(self).closest('tr').find('.item-total').val(null);
        insertQty(self);
    }
    function insertQty(self) {
       ...
       // alert(qty)
        ...
        var stock = $(self).closest('tr').find('.item-stock').val();
       
        if(parseFloat(qty) > parseFloat(stock)){
            qty = stock;
            $(self).val(qty);
            alert('You input maximum qty.');
        }
        var total = 0;
        if(qty && rate && qty > 0){
            total = qty*rate;
    ...
    function master_function() {
        ...
        function duplicateItemCheck(){

            var values =  $('.item-product').map(function() {
                if(this.value){
                    return this.value;
                }
            }).toArray();
