...
use Luova\Payment\Http\Requests\PaymentFV;
use Luova\Payment\Models\Payment;
use Luova\Payment\Models\PaymentDetail;
...
class PaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('payment_ledger', function ($data) {
                ...
                ->addColumn('action', function ($data) {
                    ...
                    $button .= '<a href="' . route('payment.detail', $data->id) . '" class="btn btn-success btn-flat"><i class="material-icons">visibility</i></a>';
                    $button .= '<a href="' . route('payment.edit', $data->id) . '" class="btn btn-warning btn-flat"><i class="material-icons">create</i></a>';
        ...
        $fdata = null;
        $mdata = null;
        return view('payment::index', compact('mdata', 'fdata'));
    }
    ...
    {
        $fdata =  Payment::FindOrFail($id);
      
        $mdata = null;
        return view('payment::detail', compact('mdata', 'fdata'));
    }
    ...
    public function store(PaymentFV $request)
    {
        //dd($request);
        $id = $request->get('id');
        // store
        ...
        try {
            if ($id) {
               ...
               $this->PaymentPosting($request,$insert->id);
            ...
            return redirect()->route('payment.all')->with('success', 'Successfully save changed');
    ...
    public function ajax_rowitem(Request $request)
    {
        $html = null;
        if ($request->rows) {
            $html = view('payment::part.row')->with(['i' => $request->rows])->render();
        }
