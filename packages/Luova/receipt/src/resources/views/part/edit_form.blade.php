...
<tr id="{{'row-'.$i}}">
    ...
    <td>
        {{ Form::select('item['.$i.'][product]',  productFA(), $row->product_id,
            ['class' => ' form-control select2 item-product', 'placeholder' => ' Select Product','onchange'=>'selectProduct(this);']) }}
    ...
    <td class="input-group">
        ...
        <div class="input-group-prepend">
            <span class="input-group-text item-unit">{{ (isset($row->product->unit->title))?$row->product->unit->title:null }}</span>
        </div>
    </td>
    ...
    <td>
        ...
        <div class="input-group-prepend">
            <span class="input-group-text type-rate">{{$row->price_type}}</span>
        </div>
    </div>
    </td>
    <td>
