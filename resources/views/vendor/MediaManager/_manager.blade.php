{{-- component --}}
<media-manager inline-template v-cloak class="hide-native-scrollbar" :config="{{ json_encode([
        'baseUrl' => $base_url,
        ...
        'voice_start' => trans('MediaManager::messages.voice.start'),
        'voice_stop' => trans('MediaManager::messages.voice.stop'),
    ]) }}" :in-modal="{{ isset($modal) ? 'true' : 'false' }}" :hide-ext="{{ isset($hideExt) ? json_encode($hideExt) : '[]' }}" :hide-path="{{ isset($hidePath) ? json_encode($hidePath) : '[]' }}" :restrict="{{ isset($restrict) ? json_encode($restrict) : '{}' }}" :user-id="{{ config('mediaManager.enable_broadcasting') ? optional(auth()->user())->id : 0 }}" :upload-panel-img-list="{{ $patterns ?: '[]' }}">

    <div class="media-manager" :class="[
        ...
        {{-- global search --}}
        <global-search-panel :trans="trans" :file-type-is="fileTypeIs" :add-to-movable-list="addToMovableList" :in-movable-list="inMovableList" :no-scroll="noScroll" :browser-support="browserSupport">
        </global-search-panel>
                ...
                <div class="level-left">
                    ...
                    <div class="level-item">
                        <div class="field" :class="{'has-addons': !isBulkSelecting()}">
                            ...
                            <div class="control" v-if="!isBulkSelecting()">
                                <button class="button" ref="upload" :disabled="isLoading" @click.stop="toggleUploadPanel()" v-tippy="{arrow: true}" title="u">
                                    <span class="icon">
                            ...
                            {{-- new folder --}}
                            <div class="control">
                                <button class="button" :disabled="isLoading" @click.stop="createNewFolder()">
                                    <span class="icon">
                    ...
                    <div class="level-item">
                        <div class="field has-addons">
                            {{-- move --}}
                            <div class="control">
                                <button class="button is-link" ref="move" :disabled="isLoading || !movableItemsCount" v-tippy="{arrow: true}" title="m / p" @click.stop="moveItem()">
                                    <span class="icon">
                                        <icon name="share" scale="0.8"></icon>
                                    </span>
                            ...
                            {{-- rename --}}
                            <div class="control" v-if="!isBulkSelecting()">
                                <button class="button is-link" ref="rename" :disabled="ops_btn_disable" @click.stop="renameItem()">
                                    <span class="icon">
                            ...
                            <div class="control" v-show="!isBulkSelecting()">
                                <button class="button is-link" ref="editor" :disabled="editor_btn_disable" v-tippy="{arrow: true}" title="e" @click.stop="imageEditor()">
                                    <span class="icon">
                                        <icon name="object-ungroup" scale="1.2"></icon>
                                    </span>
                            ...
                            {{-- delete --}}
                            <div class="control">
                                <button class="button is-link" ref="delete" :disabled="ops_btn_disable" v-tippy="{arrow: true}" title="d / del" @click.stop="deleteItem()">
                                    <span class="icon">
                    ...
                    <div class="level-item">
                        <div class="field has-addons">
                            {{-- refresh --}}
                            <div class="control" v-if="!isBulkSelecting()">
                                <v-touch class="button is-primary" ref="refresh" :disabled="isLoading" tag="button" v-tippy="{arrow: true}" title="(R) efresh" @tap="refresh()" @hold="clearAll()">
                                    <span class="icon">
                            ...
                            {{-- lock --}}
                            <div class="control">
                                <button class="button is-warning" ref="lock" :disabled="lock_btn_disable" v-tippy="{arrow: true}" title="(L) ock" @click.stop="lockFileForm()">
                                    <span class="icon">
                            ...
                            {{-- visibility --}}
                            <div class="control">
                                <button class="button" :class="IsVisible(selectedFile) ? 'is-light' : 'is-danger'" ref="visibility" :disabled="vis_btn_disable" v-tippy="{arrow: true}" title="(V) isibility" @click.stop="FileVisibilityForm()">
                                    <span class="icon">
                ...
                <div class="level-right" v-if="!waitingForUpload">
                    <div class="level-item">
                        <div class="field" :class="{'has-addons' : isBulkSelecting()}">
                            {{-- bulk select all --}}
                            <div class="control">
                                <button @click.stop="blkSlctAll()" ref="bulkSelectAll" class="button" :class="{'is-warning' : bulkSelectAll}" v-show="isBulkSelecting()" :disabled="searchItemsCount == 0 || isLoading" v-tippy="{arrow: true}" title="a">
                            ...
                            {{-- bulk select --}}
                            <div class="control">
                                <button @click.stop="blkSlct()" ref="bulkSelect" class="button" :class="{'is-danger' : bulkSelect}" :disabled="searchItemsCount == 0 || !allItemsCount || isLoading" v-tippy="{arrow: true}" title="b">
                                    <span class="icon">
                    ...
                    <template>
                        ...
                        <div class="level-item" v-if="searchItemsCount != 0 && allItemsCount">
                            <filter-and-sorting :disabled="isLoading" :filter-name-is="filterNameIs" :sort-name-is="sortNameIs" :set-filter-name="setFilterName" :set-sort-name="setSortName" :have-a-file-of-type="haveAFileOfType" :trans="trans">
                            </filter-and-sorting>
                        </div>

                        {{-- dir bookmarks --}}
                        <div class="level-item" v-if="!restrictModeIsOn && firstRun">
                            <dir-bookmarks :disabled="isLoading" :dir-bookmarks="dirBookmarks" :path="files.path" :trans="trans">
                            </dir-bookmarks>
                        </div>
                            ...
                            <div class="control">
                                <div class="field has-addons">
                                    ...
                                    <p class="control" v-if="!restrictModeIsOn">
                                        <global-search-btn route="{{ route('media.global_search') }}" :is-loading="isLoading" :trans="trans" :show-notif="showNotif">
                                        </global-search-btn>
                                    </p>
                                    ...
                                    <p class="control has-icons-left">
                                        <input class="input" :disabled="isLoading" type="text" ref="search" v-model="searchFor" :placeholder="trans('find')">
                                    ...
                                    {{-- clear --}}
                                    <p class="control">
                                        <button class="button is-black" :disabled="!searchFor" v-tippy="{arrow: true}" title="{{ trans('MediaManager::messages.clear', ['attr' => trans('MediaManager::messages.search.main')]) }}" @click.stop="resetInput('searchFor')">
                                            <span class="icon">
        ...
        <section>
            <div class="media-manager__dz" :class="{'__dz-active': uploadArea}">
                <form id="new-upload" :style="uploadPanelImg">
                    ...
                    {{-- randomNames --}}
                    <div class="form-switcher" title="{{ trans('MediaManager::messages.upload.use_random_names') }}" v-tippy="{arrow: true, placement: 'right'}">
                    ...
                    <div class="save_link" @click.stop="toggleModal('save_link_modal')" v-if="!restrictUpload()">
                        <span class="icon is-large" title="{{ trans('MediaManager::messages.save.link') }}" v-tippy="{arrow: true, placement: 'left'}">
                            <icon>
            ...
            <section class="__stack-container">
                ...
                <div id="uploadPreview">
                    ...
                    <div class="dz-preview-ops btn-animate extra-func-btns">
                        {{-- add more files --}}
                        <button v-tippy="{arrow: true, placement: 'left'}" title="{{ trans('MediaManager::messages.add.more', ['attr' => null]) }} (u)" @click.stop="toggleUploadPanel()" class="btn-plain">
                        ...
                        {{-- upload --}}
                        <button v-tippy="{arrow: true, placement: 'left'}" title="{{ trans('MediaManager::messages.upload.main') }} (Enter)" ref="process-dropzone" class="btn-plain">
                            <span class="icon is-large">
                                <icon>
                                    <icon name="circle" scale="2.5"></icon>
                        ...
                        {{-- reset --}}
                        <button v-tippy="{arrow: true, placement: 'left'}" title="{{ trans('MediaManager::messages.clear', ['attr' => null]) }} (Esc)" ref="clear-dropzone" class="btn-plain">
                            <span class="icon is-large">
                                <icon>
                                    <icon name="circle" scale="2.5"></icon>
                    ...
                    <template v-for="file in uploadPreviewList">
                        <keep-alive v-if="checkForUploadedFile(file.name)">
                            <upload-preview v-if="file.name == selectedUploadPreviewName" :key="file.name" :file="file" :file-type-is="fileTypeIs" :trans="trans">
                            </upload-preview>
                        </keep-alive>
                    </template>
                </div>
                ...
                <div>
                    ...
                    {{-- no files --}}
                    <v-touch id="no_files" v-show="no_files" class="no_files" @swiperight="goToPrevFolder()" @swipeleft="goToPrevFolder()" @hold="containerClick($event, 'no_files')" @dbltap="containerClick($event, 'no_files')">
                ...
                <div class="extra-func-btns">
                    ...
                    {{-- movable list --}}
                    <v-touch v-if="allItemsCount && !isLoading && !waitingForUpload" class="movable-list" v-tippy="{arrow: true, hideOnClick: false}" :title="inMovableList() ? '{{ trans('MediaManager::messages.add.added') }}' : '{{ trans('MediaManager::messages.add.list') }} (c / x)'" @tap="addToMovableList()" @dbltap="showMovableList()" @hold="clearMovableList()">
                        <span class="icon is-large">
                ...
                {{-- files box --}}
                <v-touch class="__stack-files mm-animated" :class="{'__stack-sidebar-hidden' : !infoSidebar}" ref="__stack-files" @swiperight="goToPrevFolder()" @swipeleft="goToPrevFolder()" @hold="containerClick($event)" @dbltap="containerClick($event)" @pinchin="containerClick($event)">
                    ...
                    <ul class="__files-boxs" ref="filesList">
                        <li v-for="(file, index) in orderBy(filterBy(allFiles, searchFor, 'name'), sortName, sortDirection)" :key="file.name" :data-file-index="index" @click.stop="setSelected(file, index, $event)">
                            <v-touch class="__file-box mm-animated" :class="{'bulk-selected': IsInBulkList(file), 'selected' : isSelected(file)}" @swipeup="swipGesture($event, file, index)" @swipedown="swipGesture($event, file, index)" @swiperight="swipGesture($event, file, index)" @swipeleft="swipGesture($event, file, index)" @hold="pressGesture($event, file, index)" @dbltap="pressGesture($event, file, index)">

                                {{-- lock file --}}
                                <button v-if="$refs.lock" class="__box-lock-icon icon" :disabled="isLoading" :class="IsLocked(file) ? 'is-danger' : 'is-success'" :title="IsLocked(file) ? '{{ trans('MediaManager::messages.unlock') }}' : '{{ trans('MediaManager::messages.lock') }}'" v-tippy="{arrow: true, hideOnClick: false}" @click.stop="lockFileForm(file)">
                                </button>

                                {{-- copy file link --}}
                                <div v-if="!fileTypeIs(file, 'folder')" class="__box-copy-link icon" @click.stop="copyLink(file.path)" :title="linkCopied ? trans('copied') : trans('to_cp')" v-tippy="{arrow: true, hideOnClick: false}" @hidden="linkCopied = false">
                                ...
                                <div class="__box-data">
                                    <div class="__box-preview">
                                        <template v-if="fileTypeIs(file, 'image')">
                                            <image-intersect :file="file" :check-for-dimensions="checkForDimensions" :browser-support="browserSupport" root-el=".__stack-files">
                                            </image-intersect>
                                        </template>
                    ...
                    <infinite-loading v-if="firstRun && files.next" spinner="waveDots" @infinite="loadPaginatedFiles">
                ...
                {{-- info sidebar --}}
                <v-touch v-if="infoSidebar" class="__stack-sidebar is-hidden-touch" @swiperight="toggleInfoSidebar(), saveUserPref()" @swipeleft="toggleInfoSidebar(), saveUserPref()">
                    ...
                    <div class="__sidebar-preview">
                        <transition name="mm-slide" mode="out-in" appear>
                            ...
                            <div key="none-selected" class="__sidebar-none-selected" v-if="!selectedFile">
                                <span @click.stop="reset()" class="link">
                                    <icon name="power-off" scale="3.2"></icon>
                                </span>
                                <p>{{ trans('MediaManager::messages.select.nothing') }}</p>
                            </div>

                            {{-- img --}}
                            <image-preview v-else-if="selectedFileIs('image')" v-tippy="{arrow: true, placement: 'left'}" title="{{ trans('MediaManager::messages.space') }}" :key="selectedFile.storage_path">

                                <img :src="selectedFile.path" :alt="selectedFile.name" class="link image" @click.stop="isBulkSelecting() ? false : toggleModal('preview_modal')" />
                            </image-preview>

                            {{-- video --}}
                            <div v-else-if="selectedFileIs('video')" v-tippy="{arrow: true, placement: 'left'}" title="{{ trans('MediaManager::messages.space') }}" :key="selectedFile.storage_path">
                                <video controls playsinline @loadedmetadata="saveVideoDimensions" preload="metadata" data-player :src="selectedFile.path">
                                    {{ trans('MediaManager::messages.video_support') }}
                                </video>
                            </div>

                            {{-- audio --}}
                            <div v-else-if="selectedFileIs('audio')" v-tippy="{arrow: true, placement: 'left'}" title="{{ trans('MediaManager::messages.space') }}" :key="selectedFile.storage_path">
                                <template>
                                    <img v-if="audioFileMeta && audioFileMeta.cover" :src="audioFileMeta.cover" :alt="selectedFile.name" class="image" />
                                    <icon v-else class="svg-prev-icon" name="music" scale="8"></icon>
                                </template>

                                <audio controls class="is-hidden" preload="none" data-player :src="selectedFile.path">
                                    {{ trans('MediaManager::messages.audio.support') }}
                                </audio>
                            </div>

                            {{-- icons --}}
                            <div key="pdf" v-else-if="selectedFileIs('pdf')" class="link" v-tippy="{arrow: true, placement: 'left'}" title="{{ trans('MediaManager::messages.space') }}" @click.stop="toggleModal('preview_modal')">
                                <icon class="svg-prev-icon" name="file-pdf-o" scale="4"></icon>
                            </div>

                            <div key="text" v-else-if="selectedFileIs('text')" class="link" v-tippy="{arrow: true, placement: 'left'}" title="{{ trans('MediaManager::messages.space') }}" @click.stop="toggleModal('preview_modal')">
                                <icon class="svg-prev-icon" name="file-text-o" scale="4"></icon>
                            </div>

                            <icon-types v-else classes="svg-prev-icon" :file="selectedFile" :file-type-is="fileTypeIs" :scale="4" :except="['image', 'audio', 'video', 'pdf', 'text']" />
                        </transition>
                    </div>

                    {{-- info --}}
                    <div v-if="allItemsCount" class="__sidebar-info" :style="{'background-color': selectedFile ? 'white' : ''}">
                                                    ...
                                                    <td class="t-val">
                                                        <form action="{{ route('media.folder_download') }}" method="post" @submit.prevent="ZipDownload($event)">
                                                            ...
                                                            <button type="submit" class="btn-plain zip" :disabled="config.gfi && selectedFile.count == 0">
                                                                <span class="icon">
                                                                    <icon name="archive" scale="1.2"></icon>
                                                                </span>
                                                            </button>
                                                        </form>
                                                    </td>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="t-val">
                                                        <button class="btn-plain" @click.prevent="saveFile(selectedFile)">
                                                            <span class="icon">
                                                                <icon name="download" scale="1.2"></icon>
                                                            </span>
                                                        </button>
                                                    </td>
                                                    {{-- zip --}}
                ...
                <v-touch v-else-if="!infoSidebar && !isASmallScreen" class="__sidebar-swipe-hidden" @swiperight="toggleInfoSidebar(), saveUserPref()" @swipeleft="toggleInfoSidebar(), saveUserPref()">
                </v-touch>
            </section>
                    ...
                    <div class="level-item">
                        <nav class="breadcrumb has-arrow-separator is-hidden-touch">
                            <transition-group tag="ul" name="mm-list">
                                <li key="library-bc">
                                    <a v-if="pathBarDirsList.length > 0 && !(isBulkSelecting() || isLoading)" class="p-l-0 level" v-tippy="{arrow: true}" title="{{ trans('MediaManager::messages.backspace') }}" @click.stop="goToFolder(0)">
                                        <span class="icon level-item is-marginless">
                                            <icon name="map"></icon>
                                        </span>
                                ...
                                <li v-for="(folder, index) in pathBarDirsList" :key="`${index}-${folder}`">
                                    <p v-if="isLastItemByIndex(index, pathBarDirsList) || isBulkSelecting() || isLoading" class="level">
                                        <span class="icon level-item is-marginless">
                                            <icon name="folder-open-o"></icon>
                                        </span>
                                        <span class="level-item m-l-5 is-marginless">@{{ folder }}</span>
                                    </p>
                                    <a v-else v-tippy="{arrow: true}" title="{{ trans('MediaManager::messages.backspace') }}" class="level" @click.stop="pathBarDirsList.length > 1 ? goToFolder(index+1) : false">
                                        <span class="icon level-item is-marginless">
                                            <icon name="folder"></icon>
                                        </span>
                                        <span class="level-item m-l-5 is-marginless">@{{ folder }}</span>
                ...
                <div class="level-right">
                    ...
                    <div class="level-item" v-if="waitingForUpload">
                        <nav class="breadcrumb">
                            <ul>
                                <li>
                                    <p class="level has-text-weight-bold">@{{ uploadPreviewList.length }} File's</p>
                                </li>
                                <li>
                                    <p class="level has-text-weight-bold">@{{ uploadPreviewListSize }}</p>
                                </li>
                            </ul>
                        </nav>
                    </div>
                    ...
                    <div class="level-item" v-show="!isLoading && !waitingForUpload">
                        <div class="is-hidden-touch" @click.stop="toggleInfoSidebar(), saveUserPref()" v-tippy="{arrow: true}" title="t" v-if="allItemsCount">
                        ...
                        <div class="is-hidden-desktop">
                            <button class="button is-link __stack-left-toolbarToggle" @click.stop="toolBar = !toolBar">
                                <span class="icon">
                                    <icon :name="toolBar ? 'times' : 'bars'"></icon>
                                </span>
                            </button>
                        </div>
                    </div>
